...
const Services = () => {
  ...
  <img className='bg-dark_green w-1/4 lg:w-1/5  object-contain md:h-full p-5 rounded-tl-lg rounded-bl-lg' src={service.icon}/>
    <div className="flex flex-col relative justify-between font-raleway gap-2 m-3">
      ...
      {service.id&&<motion.div 
         ...
         transition={{ duration: 0.8, ease: "easeInOut" }}
            ...
            className='fixed inset-0 flex items-center justify-center transition-all backdrop-blur-sm h-full '>
            <div className=' m-10 rounded-lg p-8 bg-dark_green'>
            <div className='flex flex-row justify-between items-center gap-3'>
            <div className='flex flex-row gap-4 items-center'>
            <img src={service.icon} className='w-24'/>
            <h1 className='text-white font-semibold text-2xl '>{service.title}</h1>
            </div>
